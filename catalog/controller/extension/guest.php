...
class ControllerExtensionGuest extends Controller {
	...
	public function index(){
		...
		foreach($data['tmdqc_guests'] as $form) {
		...
		$this->response->setOutput($this->load->view('extension/guest', $data));
	...
	if($feild=='tmdqc_guest_lastname'){
		...
		$newfelid =  '<div class="form-group'.$required.$status.' col-md-6 col-lg-6 padd"><label class="control-label" for="input-payment-lastname">'.$label .'</label>
	...
	if($feild=='tmdqc_guest_country'){
			...
			$newfelid =	'<div class="form-group '.$required.$status.' col-md-6 col-lg-6 padd"><label class="control-label" for="input-payment-country">'.$label.'</label><select name="country_id" id="input-payment-country" class="form-control"><option value="">'. $alldata['text_select'] .'</option>';
