<?php
class ControllerExtensionPaymentMethod extends Controller {
	public function index() {
		...
		if ($this->config->get('tmdqc_checkout_id')) {
			...
			if ($information_info) {
				$data['text_agree'] = sprintf($this->language->get('text_agree'), $this->url->link('information/information/agree', 'information_id=' . $this->config->get('tmdqc_checkout_id'), true), $information_info['title'], $information_info['title']);
			} else {
				$data['text_agree'] = '';
			}
		} else {
		...
		$this->response->setOutput($this->load->view('extension/payment_method', $data));
	}
