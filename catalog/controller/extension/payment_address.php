<?php
class ControllerExtensionPaymentAddress extends Controller {
	public function index() {
		...
		$this->response->setOutput($this->load->view('extension/payment_address', $data));
	...
	public function loadform() {
		...
		$data['shipping_required'] = $this->cart->hasShipping();
		$fileds='';
		foreach($data['tmdqc_billaddresses'] as $form) {
		...
		$this->response->setOutput($this->load->view('extension/address', $data));
	...
	private function createformbysort($feild,$label,$required,$status,$alldata) {
		...
		if($feild=='tmdqc_billaddress_firstname'){
			$newfelid =  '<div class="form-group'.$required.$status.' col-md-6 col-lg-6">
		...
		if($feild=='tmdqc_billaddress_country'){
				$newfelid =	'<div class="form-group '.$required.$status.' col-md-6 col-lg-6"><label class="control-label" for="input-payment-country">'.$label.'</label><select name="country_id" id="input-payment-country" class="form-control"><option value="">'. $alldata['text_select'] .'</option>';
