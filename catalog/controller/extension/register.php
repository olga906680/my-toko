<?php
class ControllerExtensionRegister extends Controller {
	public function index() {
		...
		if ($this->config->get('tmdqc_account_id')) {
			...
			if ($information_info) {
				$data['text_agree'] = sprintf($this->language->get('text_agree'), $this->url->link('information/information/agree', 'information_id=' . $this->config->get('tmdqc_account_id'), true), $information_info['title'], $information_info['title']);
			} else {
				$data['text_agree'] = '';
			}
		} else {
		...
		$fileds='';
		foreach($data['tmdqc_registers'] as $form) {
		...
		$this->response->setOutput($this->load->view('extension/register', $data));
	}
	...
	private function createformbysort($feild,$label,$required,$status,$alldata){
		...
		if($feild=='tmdqc_register_firstname'){
			$newfelid =  '<div class="form-group'.$required.$status.' col-md-6 col-lg-6 padd">
		...
		if($feild=='tmdqc_register_newsletter'){
				$newfelid = '<div class="checkbox '.$required.$status.' col-md-12 col-lg-12 padd" id="input-payment-newsletter">
					 <input type="checkbox" name="newsletter" value="1" class="checkbox1"/>
					 <label for="input-payment-newsletter">'.$label.'</label>
					</div>';
				return $newfelid;
		}
	}
