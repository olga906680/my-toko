<?php
class ControllerExtensionGuestShipping extends Controller {
	public function index() {
		...
		$this->response->setOutput($this->load->view('extension/guest_shipping', $data));
	}
	...
	private function createformbysort($feild,$label,$required,$status,$alldata) {
		...
		if($feild=='tmdqc_deliveryaddress_firstname'){
			$newfelid =  '<div class="form-group'.$required.$status.' col-md-12">
		...
		if($feild=='tmdqc_deliveryaddress_country'){
				$newfelid =	'<div class="form-group '.$required.$status.' col-md-12"><label class="control-label" for="input-shipping-country">'.$label.'</label><select name="country_id" id="input-shipping-country" class="form-control"><option value="">'. $alldata['text_select'] .'</option>';
