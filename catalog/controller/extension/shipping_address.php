<?php
class ControllerExtensionShippingAddress extends Controller {
	public function index(){
		...
		$this->response->setOutput($this->load->view('extension/shipping_address', $data));	
		
		/* if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/shipping_address')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/shipping_address', $data));
		} else {
			$this->response->setOutput($this->load->view('extension/shipping_address', $data));
		} */
	}
	...
	public function loadform(){
		...
		$fileds='';
		foreach($data['tmdqc_deliveryaddresses'] as $form) {
			$fileds .=$this->createformbysort($form['feild'],$form['label'][$this->config->get('config_language_id')],$form['required'],$form['status'],$data['alldata']);
		}
		$data['fileds']=$fileds;
			
		$this->response->setOutput($this->load->view('extension/shippingaddress_form', $data));
		
		/* if(file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/shippingaddress_form')){
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/shippingaddress_form', $data));
		}else{
			$this->response->setOutput($this->load->view('extension/shippingaddress_form', $data));
		} */
	}
	...
	private function createformbysort($feild,$label,$required,$status,$alldata) {
