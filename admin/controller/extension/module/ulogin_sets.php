...
class ControllerExtensionModuleUloginSets extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/ulogin_sets', 'user_token=' . $this->session->data['user_token'] . '&type=module', true)
		);

		$data['action'] = $this->url->link('extension/module/ulogin_sets', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
		...
		$this->response->setOutput($this->load->view('extension/module/ulogin_sets', $data));
	}
...
$this->load->model('setting/event');
		...
		$this->model_setting_event->addEvent('ulogin_sets', 'catalog/controller/account/logout/after', 'extension/module/ulogin_sets/logout');
		//создание группы uLogin
		if(file_exists(DIR_APPLICATION . 'model/customer/customer_group.php')){
			$this->load->model('customer/customer_group');
			$modelName = 'model_customer_customer_group';
		} else {
			$this->load->model('sale/customer_group');
			$modelName = 'model_sale_customer_group';
		}
		$this->load->model('localisation/language');
		$this->load->model('setting/setting');

		$groups = $this->$modelName->getCustomerGroups();
		...
		$default_group = $this->$modelName->getCustomerGroup($default_group_id);
		...
		$this->$modelName->addCustomerGroup($data);
