	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('ulogin', $this->request->post);
			} else {
		...
		$data['text_module_description'] = sprintf($this->language->get('text_module_description'), $this->url->link('extension/module/ulogin_sets', 'user_token=' . $this->session->data['user_token'] . '&type=module', true));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/ulogin', 'user_token=' . $this->session->data['user_token'] . '&type=module', true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/ulogin', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/ulogin', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
		} else {
			$data['action'] = $this->url->link('extension/module/ulogin', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('extension/module/ulogin', $data));
	}
...
$this->load->model('setting/event');
		...
		$this->model_setting_event->addEvent('ulogin', 'catalog/controller/account/logout/after', 'extension/module/ulogin/logout');
		// созание модулей с настройками по умолчанию
		$this->model_setting_module->addModule('ulogin', array(
	...
	public function uninstall() {
		$this->load->model('setting/event');
		$this->model_setting_event->deleteEventByCode('ulogin');
	}
}